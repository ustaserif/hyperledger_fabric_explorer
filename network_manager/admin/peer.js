...
export default class Peer {
	...
	constructor({peerPort, peerHostName, cert, pem, host, clientKey, clientCert, mspid}, logger = console) {
		...
		const options = RemoteOptsTransform({
			...
			clientKey: this.clientKey && (fs.existsSync(this.clientKey) ? fs.readFileSync(this.clientKey).toString() : this.clientKey),
			clientCert: this.clientCert && (fs.existsSync(this.clientCert) ? fs.readFileSync(this.clientCert).toString() : this.clientCert),
		});
