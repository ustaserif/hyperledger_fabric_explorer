...
export default class FabricCAService {
	...
	async enroll(req, {subject, dns = []} = {}) {
		...
		if (!csr) {
			...
			csr = keyPair.generateCSR({str: asn1.x509.X500Name.ldapToOneline(subject)}, [extension]);
		}
	 ...
	 * 
	 */
	async register(req, _signingIdentity) {
		const {enrollmentID, enrollmentSecret, role, affiliation, maxEnrollments, attrs} = req;
		
		const result = {};
		
		const secret = await this._fabricCAClient.register(
			enrollmentID, 
			enrollmentSecret,
			role,
			affiliation,
